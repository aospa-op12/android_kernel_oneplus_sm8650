...
#if defined(FTS_DRM_PANEL_NOTIFY) && (FTS_DRM_PANEL_NOTIFY == 1)
#include <drm/drm_panel.h>
#elif defined(FTS_MSM_DRM_PANEL_NOTIFY) && (FTS_MSM_DRM_PANEL_NOTIFY == 1)
#include <linux/msm_drm_notify.h>
#elif defined(FTS_FB_NOTIFY) && (FTS_FB_NOTIFY == 1)
#include <linux/notifier.h>
#include <linux/fb.h>
#elif defined(FTS_QCOM_PANEL_EVENT_NOTIFY) && (FTS_QCOM_PANEL_EVENT_NOTIFY == 1)
#include <drm/drm_panel.h>
...
#endif
            input_report_abs(input_dev, ABS_MT_TOUCH_MAJOR, events[i].major);
...
{
    if (!ts_data || !events || !ts_data->input_dev) {
        FTS_ERROR("ts_data/events/input_dev is null");
        return -EINVAL;
    }
...
int fts_input_report_pen(struct fts_ts_data *ts_data, struct pen_event *pevt)
{
    ...
    if (!ts_data || !pevt || !ts_data->pen_dev) {
        FTS_ERROR("ts_data/pevt/pen_dev is null");
        return -EINVAL;
    }

    mutex_lock(&ts_data->report_mutex);
    input_report_key(pen_dev, BTN_STYLUS, !!(pevt->btn_val & 0x02));
    input_report_key(pen_dev, BTN_STYLUS2, !!(pevt->btn_val & 0x08));
    ...
    case STYLUS_HOVER:
        if (ts_data->log_level >= 1)
            FTS_DEBUG("[PEN][%02X]x:%d,y:%d,p:%d,tip:%d,flag:%d,tilt:%d,%d,%d",
                      pevt->btn_val, pevt->x, pevt->y, pevt->p, pevt->tip,
        ...
        input_report_abs(pen_dev, ABS_TILT_X, pevt->tilt_x);
        input_report_abs(pen_dev, ABS_TILT_Y, pevt->tilt_y);
        input_report_key(pen_dev, BTN_TOOL_PEN, EVENT_DOWN(pevt->flag));  //BTN_DIGI hover
        input_report_key(pen_dev, BTN_TOUCH, pevt->tip);  //BTN_TOUCH
        input_sync(pen_dev);
        break;
    default:
...
static int fts_input_get_report_pen(struct fts_ts_data *ts_data, u8 *pen_buf)
{
    struct pen_event *pevt = &ts_data->pevent;

    /*get information of stylus*/
    pevt->btn_val = pen_buf[2];
    ...
    fts_input_report_pen(ts_data, pevt);
    return 0;
}

static int fts_input_get_report_pen_pv21(struct fts_ts_data *ts_data, u8 *pen_buf)
{
    struct pen_event *pevt = &ts_data->pevent;
    int hires_tp = 0;
    int hires_host = FTS_PEN_HIRES_EN ? FTS_PEN_HIRES_X : 1;

    /*get information of stylus*/
    hires_tp = ((pen_buf[FTS_STRUCT_VER] >> 4) & 0x0F) + 1;
    pevt->btn_val = pen_buf[4];
    pevt->inrange = (pen_buf[4] & 0x20) ? 1 : 0;
    pevt->tip = (pen_buf[4] & 0x01) ? 1 : 0;
    pevt->flag = pen_buf[5] >> 6;
    pevt->id = pen_buf[5] >> 4;
    pevt->x = (((pen_buf[6] << 8) + pen_buf[7]) * hires_host) / hires_tp;
    pevt->y = (((pen_buf[8] << 8) + pen_buf[9]) * hires_host) / hires_tp;
    pevt->p = (pen_buf[10] << 8) + pen_buf[11];
    pevt->tilt_x = (short)((pen_buf[12] << 8) + pen_buf[13]);
    pevt->tilt_y = (short)((pen_buf[14] << 8) + pen_buf[15]);
    pevt->azimuth = (pen_buf[16] << 8) + pen_buf[17];
    pevt->speed = (pen_buf[18] << 8) + pen_buf[19];
    pevt->battery = pen_buf[20];
...
static int fts_input_get_report_touch(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    int i = 0;
    int event_num = 0;
    int finger_num = 0;
...
#endif
        events[i].major = touch_buf[FTS_TOUCH_OFF_AREA + base];
        if (events[i].major <= 0) events[i].major = 0x09;
        events[i].minor = events[i].major;
    ...
    fts_input_report_touch(ts_data, events);
    return 0;
}

static int fts_input_get_report_touch_pv2(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    int i = 0;
    int event_num = 0;
    int pointid = 0;
...
static int fts_input_get_report_touch_pv30(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    int i = 0;
    int event_num = 0;
    int pointid = 0;
    ...
    int max_touch_num = ts_data->pdata->max_touch_number;
    int hires_tp = 0;
    int hires_host = FTS_TOUCH_HIRES_EN ? FTS_TOUCH_HIRES_X : 1;
    struct ts_event *events = ts_data->events;
    ...
    ts_data->touch_event_num = event_num;
    hires_tp = ((touch_buf[FTS_STRUCT_VER] >> 4) & 0x0F) + 1;
    for (i = 0; i < event_num; i++) {
        base = FTS_ONE_TCH_LEN_V2 * i + 4;
        pointid = (touch_buf[base]) & 0x0F;
        if (pointid >= max_touch_num) {
        ...
        events[i].id = pointid;
        events[i].flag = touch_buf[base] >> 6;
        events[i].x = (((touch_buf[1 + base] << 8) + touch_buf[2 + base]) * hires_host) / hires_tp;
        events[i].y = (((touch_buf[3 + base] << 8) + touch_buf[4 + base]) * hires_host) / hires_tp;
        events[i].major = touch_buf[5 + base];
        events[i].minor = touch_buf[6 + base];
        events[i].misc = touch_buf[7 + base];
        events[i].p = 0x3F;
#if FTS_REPORT_PRESSURE_EN
        FTS_ERROR("The pressure property isn't supported");
#endif
...
int fts_input_report_buffer(struct fts_ts_data *ts_data, u8 *report_buf)
{
    ...
    touch_etype_sub = (report_buf[FTS_STRUCT_VER] & 0x0F);
    switch (touch_etype) {
    case TOUCH_DEFAULT:
        ret = fts_input_get_report_touch(ts_data, report_buf);
        break;

    case TOUCH_PROTOCOL_v2:
        ret = fts_input_get_report_touch_pv2(ts_data, report_buf);
        break;

    case TOUCH_PROTOCOL_v3:
        switch (touch_etype_sub) {
        case 0x00:
            ret = fts_input_get_report_touch_pv30(ts_data, report_buf);
            break;
        default:
            FTS_INFO("unknown touch event(%d-%d)", touch_etype, touch_etype_sub);
            break;
        }
        break;
...
#if FTS_PEN_EN
    case TOUCH_PEN:
        ret = fts_input_get_report_pen(ts_data, report_buf);
        break;

    case TOUCH_PEN_v2:
        switch (touch_etype_sub) {
        case 0x01:
            ret = fts_input_get_report_pen_pv21(ts_data, report_buf);
            break;
        default:
            FTS_INFO("unknown touch event(%d-%d)", touch_etype, touch_etype_sub);
            break;
...
static int fts_read_parse_touchdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    if (ret < 0) {
        FTS_ERROR("read touch data failed, ret=%d", ret);
        return TOUCH_ERROR;
    }
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
    ...
    if ((touch_etype >= TOUCH_DEFAULT) && (touch_etype < TOUCH_FWDBG)) {
...
static irqreturn_t fts_irq_handler(int irq, void *data)
{
    ...
    ts_data->isr_state = ISR_EXECUTION;
    ...
    ts_data->isr_state = ISR_TERMINATION;
    return IRQ_HANDLED;
}
...
static int fts_pinctrl_init(struct fts_ts_data *ts)
{
    ...
    if (IS_ERR_OR_NULL(ts->pins_release)) {
        FTS_ERROR("Pin state[release] not found,err=%ld", PTR_ERR(ts->pins_release));
...
#if (IS_ENABLED(FTS_DRM_PANEL_NOTIFY) || IS_ENABLED(FTS_QCOM_PANEL_EVENT_NOTIFY) )
...
#ifdef FTS_QCOM_PANEL_EVENT_NOTIFY
