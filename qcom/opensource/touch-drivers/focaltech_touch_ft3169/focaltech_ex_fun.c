    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (ts_data->bus_type != BUS_TYPE_I2C) {
        count += snprintf(buf + count, PAGE_SIZE, "SPI speed, ori:%d,now:%d\n", ts_data->spi_speed, ts_data->spi->max_speed_hz);
    }
...
static ssize_t fts_spi_speed_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    ...
    if (ts_data->bus_type == BUS_TYPE_I2C) {
        count += snprintf(buf + count, PAGE_SIZE, "I2C driver not support\n");
        return count;
    }
    mutex_lock(&input_dev->mutex);
    count += snprintf(buf + count, PAGE_SIZE, "spi speed:%d\n",
...
static ssize_t fts_spi_speed_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    int value = 0, ret = 0;
    ...
    FTS_FUNC_ENTER();
    if (ts_data->bus_type == BUS_TYPE_I2C) {
        FTS_ERROR("I2C driver not support");
        return count;
    }
    mutex_lock(&input_dev->mutex);
    sscanf(buf, "%d", &value);

    FTS_DEBUG("spi speed:%d->%d", ts_data->spi->max_speed_hz, value);
    ts_data->spi->max_speed_hz = value;
...
static DEVICE_ATTR(fts_spi_speed, S_IRUGO | S_IWUSR, fts_spi_speed_show, fts_spi_speed_store);
#if FTS_FOD_EN
...
int fts_create_sysfs(struct fts_ts_data *ts_data)
{
    ...
    ret = sysfs_create_link(NULL, &ts_data->dev->kobj, "fts_ts");
    if (ret < 0) {
        FTS_ERROR("[EX]: sysfs_create_link() failed!!");
    }
